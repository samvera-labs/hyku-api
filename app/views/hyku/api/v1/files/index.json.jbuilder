...
json.cache_root! [@account.tenant, @work_document.id, @work_document[:system_modified_dtsi], :files] do
  json.array! @files.each do |file|
    json.cache! [@account.tenant, @work_document, :files, file.id, file.solr_document[:system_modified_dtsi]] do
      json.uuid file.id
      json.type    'file_set'
